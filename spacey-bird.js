class Pipe extends Path2D {
    constructor(game, pipeWidth = 50, pipeHeight = 80, xPosition = 300, speed = 3, color = "black") {
        super();
        this.game = game;
    ...
    draw(ctx) {
        ...
        ctx.fillRect(this.xPosition, this.gameHeight - this.pipeHeight, this.pipeWidth, this.pipeHeight);
...
class Game {
    ...
    init() {
        ...
        this.canvas.focus();
        document.getElementById("gameContainer").setAttribute("width",this.gameWidth);
        document.getElementById("gameContainer").setAttribute("height",this.gameHeight);
        this.draw();
    }
    ...
    draw() {
        ...
        if (this.getGameState() == this.GAME_STATES.MENU) {
            let menuEntries = [new MenuEntry ("START", () =>  this.startGame()),
            new MenuEntry ("Credits", () =>  console.log("Credits"))
            ]
            let menu = new Menu(this.canvas, this.ctx, this, () => this.draw(), menuEntries, "Welcome to Spacey Bird");
            menu.draw(this.ctx);
        }
        ...
        if (this.getGameState() == this.GAME_STATES.PLAY) {
            if (this.debug) {
                ...
                this.fpsCtx.fillText(`${this.player.playerX}, ${Number.parseInt(this.player.playerY)}, ${Number.parseInt(this.player.verticalAcceleration)}`,250,20);
            }
        ...
        if (this.getGameState() == this.GAME_STATES.END) {
            let menuEntries = [
                ...
                new MenuEntry ("Retry", () => { this.startGame()})
            ];

            let endScreen = new Menu(this.canvas, this.ctx, this, () => this.draw(), menuEntries, `Your score: ${this.score}`);
...
class Menu extends Path2D {
    ...
    draw(ctx) {
        ...
        for (let menuEntryId in this.menuEntries) {
            ...
            ctx.fillText(this.menuEntries[menuEntryId].title, 175,150+menuEntryId*30)
        }
        ctx.stroke();
    }
...
class Player extends Path2D {
    ...
    _flapHandle = (event, canvas) => {this.flap()};
    draw (ctx) {
     ...
     * The first coordinate is top-left, the second the nose, the third is the bottom-left coordinate.
     */
    getHitBox() {
        let hitbox = [[this.playerX, this.playerY],[this.playerX+35, this.playerY+17.5], [this.playerX, this.playerY+35]];
        return hitbox;
    }
    move() {
...
class SoundManager {
    ...
    registerSoundEvent(event, sound) {
        ...
        this.canvas.addEventListener(
            event.type, () => new Audio(selectedSound).play()
        );
    }
}
